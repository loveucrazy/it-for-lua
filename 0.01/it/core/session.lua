...
function session.new(opts)
    ...
    local o, i = pcall(require, "session.storage." .. (y.storage or z.storage))
    if not o then
        i = require "session.storage.cookie"
    end
    local o, j = pcall(require, "session.serializers." .. (y.serializer or z.serializer))
    if not o then
        j = require "session.serializers.json"
    end
    local o, k = pcall(require, "session.encoders." .. (y.encoder or z.encoder))
    if not o then
        k = require "session.encoders.base64"
    end
    local o, l = pcall(require, "session.ciphers." .. (e or f))
    if not o then
        l = require "session.ciphers.aes"
    end
    local self = {
